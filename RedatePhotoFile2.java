/*
  Redate Photo File #2 - Change File Dates, Names for JPEG Photos
  Written by: Keith Fenske, http://kwfenske.github.io/
  Wednesday, 6 September 2017
  Java class name: RedatePhotoFile2
  Copyright (c) 2017 by Keith Fenske.  GNU General Public License (GPLv3+).
  ...
  --------------------------------
  RedatePhotoFile2 is free software: you can redistribute it and/or modify it
  ...
  with the "2>" operator.)  An example command line is:
      ...
      java  RedatePhotoFile2  -s  d:\temp  >report.txt
...
public class RedatePhotoFile2
{
  ...
  static long dateLimitLower;     // the oldest Java time stamp we accept
  static long dateLimitUpper;     // the newest Java time stamp we accept
  ...
  static boolean showFailureFlag; // true if we show failures to change
  static boolean showOtherFlag;   // true if we show various other results
  static boolean showSuccessFlag; // true if we show successful changes
...
*/
  public static void main(String[] args)
  {
    ...
    showFailureFlag = true;       // by default, show failures to change
    showOtherFlag = true;         // by default, show various other results
    showSuccessFlag = true;       // by default, show successful changes
    ...
    /* Initialize our calendar for dates and times in the local time zone. */

    ourDateFormat = new SimpleDateFormat(DATE_FORMAT); // date, time format
    ourTimeZone = ourDateFormat.getTimeZone(); // get local time zone
    try                           // set upper and lower limits on dates
    {
      dateLimitLower = ourDateFormat.parse("1980-01-01 12:00:00").getTime();
      dateLimitUpper = ourDateFormat.parse("2099-12-31 12:00:00").getTime();
    }
    catch (ParseException pe)     // unable to parse date, time string
    {
      dateLimitLower =  316000000000L; // roughly 1980-01-06
      dateLimitUpper = 4102000000000L; // roughly 2099-12-26
    }
...
//  }
    ...
    action = new RedatePhotoFile2User(); // create our shared action listener
    ...
    oldDateCheckbox = new JCheckBox(
      "Search files for oldest (original) date and time, or else ",
      oldDateFlag);
    ...
    JPanel panel22 = new JPanel(new BorderLayout(25, 0));

    JPanel panel23 = new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 0));
    renameCheckbox = new JCheckBox(
      ("Rename files using date and time prefix, similar to: "
    ...
    panel23.add(renameCheckbox);
    panel22.add(panel23, BorderLayout.CENTER);

    renameFormatButton = new JButton("Format...");
    ...
    panel22.add(renameFormatButton, BorderLayout.EAST);

    panel01.add(panel22);
    panel01.add(Box.createVerticalStrut(4)); // space between panels
    ...
    redateCheckbox = new JCheckBox(
      "Change date in system file directory using embedded date and time.",
      redateFlag);
    if (buttonFont != null) redateCheckbox.setFont(buttonFont);
    redateCheckbox.addActionListener(action); // do last so don't fire early
    panel24.add(redateCheckbox);

    panel01.add(panel24);
    panel01.add(Box.createVerticalStrut(11)); // space between panels
    ...
    outputText.setText(
      ...
      + "\nIf in doubt, select the \"debug mode\" option."
...
*/
  static void doOpenButton()
  {
    ...
    openFilesThread = new Thread(new RedatePhotoFile2User(), "doOpenRunner");
...
*/
  ...
  {
    String bestDateTime;          // best date, time found so far, or null
...
//    cancelFlag = true;          // don't do anything more
    ...
    bestDateTime = null;          // no valid date, time found yet
                                  ...
                                  // for each byte (character) in file
      {
        ...
        switch (state)            // helps that state is also parsed length
        {
          ...
          case (19):              // found: 1999:12:31 23:59:59?
            ...
            {
              String newDateTime = new String(found);
                                  // convert character array to real string
              if (debugFlag)      // does user want details?
                putOutput(givenName + " - found date and time " + newDateTime);
              if ((bestDateTime == null) // if no date, time or new is better
                || ((oldDateFlag == false) && (bestDateTime.compareTo(newDateTime) < 0))
                || ((oldDateFlag == true)  && (bestDateTime.compareTo(newDateTime) > 0)))
              {
                bestDateTime = newDateTime; // save the better date, time
              }
...
//    cancelFlag = true;          // don't do anything more
    ...
    if (bestDateTime == null)     // won't change from null if nothing found
    {
      if (showOtherFlag)
        putOutput(givenName + " - date and time not found");
      totalNoData ++;             // one more file with no date, time info
      return;
    }
    else if (debugFlag)           // does user want details?
    {
      putOutput(givenName + " - using date and time " + bestDateTime);
    ...
    {
      if (showOtherFlag)
        putOutput(givenName + " - found date and time " + bestDateTime);
      return;
    }
    ...
    try { newMillis = ourDateFormat.parse(bestDateTime).getTime(); }
    catch (ParseException pe)     // unable to parse date, time string
    {
      putOutput(givenName + " - can't parse date and time " + bestDateTime);
      totalError ++;              // one more file with an error
      return;                     // don't continue after error
    }
    if ((newMillis < dateLimitLower) || (newMillis > dateLimitUpper))
    {
      putOutput(givenName + " - date and time not in range " + bestDateTime);
      totalError ++;              // one more file with an error
      return;                     // don't continue after error
    }
    ...
    {
      if (showOtherFlag)
        putOutput(givenName + " - has correct date and time");
      fileCorrectFlag = true;     // at least one correct for this file
    }
    else if (canon.canWrite() == false)
    {
      if (showFailureFlag)
        putOutput(givenName + " - can't change read-only file to "
          + bestDateTime);
    ...
    {
      if (showSuccessFlag)
        putOutput(givenName + " - simulate change date to " + bestDateTime);
    }
    else if (canon.setLastModified(newMillis)) // try to change date, time
    {
      if (showSuccessFlag)
      {
        /* When displaying an old date, we have the same problem as setting a
        new date above: daylight saving time (DST).  This could devolve into a
        long discussion about the difficulty of following date and time rules.
        It would be easier if we simply didn't show the old date. */

        long fromStamp = oldMillis; // may need to adjust old time stamp
        if (mswinFlag)            // do we correct for Windows assumptions?
          fromStamp -= ourTimeZone.getOffset(fromStamp)
            - ourTimeZone.getOffset((new Date()).getTime());

//      putOutput(givenName + " - changed date to " + bestDateTime);
        putOutput(givenName + " - changed date to " + bestDateTime + " from "
          + ourDateFormat.format(new Date(fromStamp)));
      }
    ...
    {
      if (showFailureFlag)
        putOutput(givenName + " - failed to change date to " + bestDateTime);
      fileErrorFlag = true;       // at least one error for this file
    }
    ...
    if (debugFlag)                // does user want details?
      putOutput(givenName + " - file name prefix is <" + prefix + ">");
    ...
    {
      if (showOtherFlag)
        putOutput(givenName + " - has correct file name prefix");
      fileCorrectFlag = true;     // at least one correct for this file
    }
    else if (canon.canWrite() == false)
    {
      if (showFailureFlag)
        putOutput(givenName + " - can't rename read-only file as " + newName);
      fileErrorFlag = true;       // at least one error for this file
    }
    else if (debugFlag)           // do we simulate the result?
    {
      if (showSuccessFlag)
        putOutput(givenName + " - simulate rename to " + newName);
    }
    else if (canon.renameTo(new File(canon.getParent(), newName)))
    {                             // try to change file name
      if (showSuccessFlag)
        putOutput(givenName + " - renamed to " + newName);
      fileChangeFlag = true;      // at least one change for this file
    }
    else                          // error from renameTo() method
    {
      if (showFailureFlag)
        putOutput(givenName + " - failed to rename as " + newName);
      fileErrorFlag = true;       // at least one error for this file
    }
...
*/
  static void showHelp()
  {
    ...
    System.err.println();
    System.err.println("  java  RedatePhotoFile2  [options]  [fileOrFolderNames]");
    System.err.println();
...
//  System.err.println("  -f0 = find dates, times but do not change file date or rename");
    ...
    System.err.println("  -m0 = find oldest (original) date and time (default)");
    System.err.println("  -m1 = -m = find newest (modified) date and time");
...
/*
  ...
  buttons, in the context of the main RedatePhotoFile2 class.
*/
    ...
    else if (source == showDialog) // which files or conditions to report
    {
      ...
      {                           // same index order as SHOW_CHOICES
        case (0):                 // show all files
          ...
          break;
        case (1):                 // show changes only
          showSuccessFlag = true;
          break;
        case (2):                 // show changes, errors
          showFailureFlag = showSuccessFlag = true;
          break;
        case (3):                 // show errors only
...
} // end of RedatePhotoFile2 class

// ------------------------------------------------------------------------- //

/*
  RedatePhotoFile2User class

  This class listens to input from the user and passes back event parameters to
  a static method in the main class.
*/

class RedatePhotoFile2User implements ActionListener, Runnable
{
  /* empty constructor */

  public RedatePhotoFile2User() { }

  /* button listener, dialog boxes, etc */

  public void actionPerformed(ActionEvent event)
  {
    RedatePhotoFile2.userButton(event);
  }
  ...
  public void run()
  {
    RedatePhotoFile2.doOpenRunner();
  }

} // end of RedatePhotoFile2User class
